...
% A Trojan soldier is a soldier natural from Troy.
% \begin{itemize}
...
%   \begin{itemize}
%     \item $\dlFont{Parent} \sqsubseteq \exists \dlFont{fatherOf}.\top \sqcup \exists \dlFont{motherOf}.\top$
%   \end{itemize}
% \end{enumerate}
...
\begin{frame}{Canonical Models for \ELbot and \ELIbot}

  \centering{
  $\Kmc \models C \sqsubseteq A$ iff $C \in A^{\Imc^{\Kmc}}$
  }
  ...
  %\Large{\color{red}INCLUDE DIAGRAM.}
...
% \begin{frame}{Primeness}

%   {\color{Plum}
%   \large{(\ELbot)} \large{$C \sqsubseteq \exists r.D \Rightarrow (C,D) \in r^{\Imc^{\Kmc}}$}}
...
\begin{frame}{Rational Domain}
  ...
  \centering{
    \begin{tikzpicture}
      ...
      \draw[dashed, rounded corners] (0.9, 3) -- (5, 3) -- (5, 0.2) -- (0.9, 0.2) -- cycle;

      \node[color=black!40] at (0, 1.3){$\dlFont{Deity}$};
...
\begin{enumerate}
  ...
  \item $(\typel{M}{\Umc}, \typel{N}{\emptyset}) \in r^{\Mintypmod}$ iff $\materialization{\Kmc} \models \setconj{M} \sqcap \materialization{\Umc} \sqsubseteq \exists r.\setconj{N}$ and $N$ is prime.
  \item $(\typel{N}{\emptyset}, \typel{M}{\Umc}) \in r^{\Mintypmod}$ iff $\materialization{\Kmc} \models \setconj{M} \sqcap \materialization{\Umc} \sqsubseteq \exists r^{-}.\setconj{N}$ and $N$ is prime. 
\end{enumerate}
...
\begin{tikzpicture}
  ...
  \draw[RedOrange,thick,fill=RedOrange!30,rounded corners] (1.5, -0.5) rectangle (2.5,0.5); %C Concept
    ...
    \node[] at (1,1.7) (N) {$\exists r.C \sqsubseteq \exists r.E$};
...
\begin{frame}{Nested Reasoning}
  ...
  %\bigskip 
  %$\Kmc \models \dlFont{Human} \definc \exists \dlFont{worships}.\dlFont{Anthropomorphic}$
